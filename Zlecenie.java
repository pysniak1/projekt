...
public class Zlecenie implements Runnable {
    ...
    public void run() {
        if (brygada != null && !kolekcjaPrac.isEmpty()) {
            boolean pracownicyZajeci = false;
            for (Prac prac : brygada.getListaPracownikow()) {
                if (prac.isWykonujeZlecenie()) {
                    pracownicyZajeci = true;
                    break;
                }
            }
            ...
            if (!pracownicyZajeci) {
                ...
                System.out.println("Nie można wykonać zlecenia,poniewaz pracownik z brygady jest zajęty.");
            }
        }
    }
